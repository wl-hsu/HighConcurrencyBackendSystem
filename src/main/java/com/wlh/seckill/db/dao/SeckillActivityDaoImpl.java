...
import com.wlh.seckill.db.mappers.SeckillActivityMapper;
...
public class SeckillActivityDaoImpl implements SeckillActivityDao {
    ...
    public boolean lockStock(long activityId) {
        ...
        if (result < 1) {
            log.error("Failed to lock inventory"); // log.error("Failed to lock inventory");
            return false;
        }
        return true;
    }
    ...
    public boolean deductStock(Long seckillActivityId) {
        ...
        if (result < 1) {
            log.error("Failed to deduct inventory"); // log.error("Deduction of inventory failed");
            return false;
        }
        return true;
    }
